...
describe('payment.succeeded webhook notification integration', () => {
  ...
  it('activates the subscription and sends one success message to User.telegramId', async () => {
    ...
    const payment = {
      ...
      currency: 'RUB',
      status: 'pending',
      paymentUiMessageId: undefined,
      processingUiMessageId: undefined,
    ...
    const gateway = {
      normalizeWebhookEvent: vi
        .fn()
        .mockReturnValue({ type: 'succeeded', providerPaymentId: 'provider-1' }),
    ...
    vi.spyOn(UserModel, 'findById')
      ...
      .mockReturnValueOnce(query(user) as never);
    vi.spyOn(PurchaseIntentModel, 'findOne').mockReturnValue(
      query({ _id: purchaseIntentId }) as never,
    );
